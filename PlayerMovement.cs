...
public class PlayerMovement : MonoBehaviour
{
	...
	public GameObject KatanaGrappler;
	...
	private Animation buttonAnimaton;
	...
	private void Update()
	{
		if(GrapplerSword.Instance.isGrappling2){
        ...
        {
			if (array[i].transform.gameObject.layer == LayerMask.NameToLayer("Object"))
			{
				grabIcon.enabled = true;
				UIManger.Instance.disCross();
			}
			if (array[i].transform.gameObject.layer == LayerMask.NameToLayer("Object") && array[i].transform.gameObject.layer == LayerMask.NameToLayer("Ground"))
			{
				grabIcon.enabled = true;
				UIManger.Instance.disCross();
			}
			if (array[i].transform.gameObject.layer == LayerMask.NameToLayer("Object") && array[i].transform.gameObject.layer == LayerMask.NameToLayer("Ground") && array[i].transform.gameObject.layer == LayerMask.NameToLayer("Enemy"))
			{
				grabIcon.enabled = true;
				UIManger.Instance.disCross();
			}
	...
	void TurnOffCamera()
	{
		if(DetectWeapons.Instance.hasGun == false){
			weaponCam.enabled = false;
			playerCam_.cullingMask |= 1 << LayerMask.NameToLayer("Gun");
			playerCam_.cullingMask |= 1 << LayerMask.NameToLayer("Sword");
		}else{
	...
	public void CheckSword()
    {
        ...
        {
			Sword_.SetActive(true);
			KatanaGrappler.SetActive(true);
        }
		if (SwordEnabled == false)
		{
			Sword_.SetActive(false);
			KatanaGrappler.SetActive(false);
		}
	}
	...
	{
		RaycastHit[] array = Physics.RaycastAll(playerCam.transform.position, playerCam.transform.forward, 8f, whatIsHittable);
		if (array.Length < 1)
		{
			return;
		}
	...
	{
		RaycastHit[] array2 = Physics.RaycastAll(playerCam.transform.position, playerCam.transform.forward, 8f, whatIsHittable);
		for (int i = 0; i < array2.Length; i++)
		{
			if (array2[i].transform.gameObject.layer == LayerMask.NameToLayer("Button") && Input.GetKeyDown(KeyCode.E))
			{
				OpenDoor.Instance.buttonIsPressed = true;
				ButtonShoot.Instance.done=true;
				MonoBehaviour.print("PressedButton");
				AudioManager.Instance.PlayButton();
				StartCoroutine(ButtonShoot.Instance.Door2());
			}
		}
	}
	...
	private void OnCollisionEnter(Collision other)
	{
		if(other.gameObject.layer != LayerMask.NameToLayer("Ground")){
			if(GrapplerSword.Instance.isGrappling2){
            	GrapplerSword.Instance.startPulling = false;
				GrapplerSword.Instance.StopGrapple();
				Sword.Instance.SwingSword();
        	}
		}
